...
namespace NMaier.SimpleDlna.Server
{
    public sealed class HttpServer : Logging, IDisposable
    {
        ...
        public void RegisterMediaServer(IMediaServer server, String listenID)
        {
            if (server == null)
            {
            ...
            foreach (NetworkInterface netInterface in NetworkInterface.GetAllNetworkInterfaces())
            {
                ...
                else if (listenID == netInterface.Id)
                {
            ...
            foreach (var address in IP.ExternalIPAddresses)
            {
                ...
                {
                    mount.AddDeviceGuid(deviceGuid, overrideIP);
                    var uri = new Uri(string.Format(
                      "http://{0}:{1}{2}",
                      overrideIP,
                      end.Port,
