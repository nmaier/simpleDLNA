    ...
    private void InitCover(TagLib.Tag tag)
    {
      ...
      if (pic != null) {
        try {
          ...
          _logger.Debug("Failed to generate thumb for " + Item.FullName, ex);
        }
      }
    }
      ...
      try {
        using (var tl = TagLib.File.Create(new TagLibFileAbstraction(Item))) {
          try {
            ...
            _logger.Debug("Failed to transpose Properties props", ex);
          }
          ...
          try {
            ...
            _logger.Debug("Failed to transpose Tag props", ex);
          }
        }
